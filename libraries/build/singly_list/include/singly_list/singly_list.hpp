...
namespace adt {
    ...
    private:
        ...
        constexpr _Node* _move_node(_Node* dst, _Node*& src) noexcept {
            dst = src;
            src = nullptr;
            return dst;
        }
        ...
        constexpr _Node* _insert_after(_Node* node, _Node* pos_node = nullptr) noexcept {
            if (node == nullptr) {
                return nullptr;
            }
        ...
        constexpr void _splice_after(_Node* pos_node, _Node* src_start, _Node* src_end, size_type& src_sz) noexcept {
            // Do nothing if there no nodes in the range (src_start, src_end)
            if (src_start->next == nullptr) {
                return;
            }

            // Start at the beginning of the list if no starting position is given
            if (pos_node == nullptr) {
                pos_node = this->head;
            }

            // If there is only one node being spliced...
            if (src_start->next == src_end) {
                _Node* pos_succ = pos_node->next;
                _Node* src_succ = src_start->next->next;

                // Insert the source node (i.e src_start->next) between the position node
                // and the position node's successor
                pos_node->next = src_start->next;
                pos_node->next->next = pos_succ;

                // Point the source node to it's successor
                src_start->next = src_succ;

                // Update the source list's size and *this's size
                src_sz--;
                this->sz++;
            ...
            // While there are nodes to splice...
            do {
                _Node* pos_succ = pos_node->next;
                _Node* src_succ = src_start->next->next;
                ...
                // Advance the position node to it's successor
                pos_node = pos_node->next;
            } while (src_start->next != src_end && src_start->next != nullptr);
        }

        constexpr _Node* _extract_after(_Node* node) noexcept {
            // Extract the node after `node`
            _Node* extracted_node = node->next;
    ...
    public:
        ...
        private:
            ...
            /* ---------------------------------------------Methods-------------------------------------------------- */
            constexpr _Node* _release() noexcept {
                _Node* released_node = this->node;
                this->node = nullptr;
        ...
        public:
            ...
            constexpr ~node_type() noexcept {
                std::allocator_traits<allocator_type>::destroy(this->allocator, this->node);
                std::allocator_traits<allocator_type>::deallocate(this->allocator, this->node, 1);
                this->node = nullptr;
            }
            ...
            [[nodiscard]] constexpr bool operator==(std::nullopt_t) const  noexcept { return this->node == nullptr; }

            [[nodiscard]] constexpr auto operator<=>(const node_type&) const noexcept = default;

            [[nodiscard]] constexpr auto operator<=>(std::nullptr_t) const noexcept { return this->node <=> nullptr; }

            [[nodiscard]] constexpr auto operator<=>(std::nullopt_t) const noexcept { return this->node <=> nullptr; }
    ...
    public:
        ...
        [[nodiscard]] constexpr auto operator<=>(const singly_list& rhs) const noexcept
            ...
            if (auto cmp = this->sz <=> rhs.sz; cmp != 0) {
                return cmp;
            }
        ...
        iterator insert_after(const_iterator pos, const_reference value)
            ...
            return iterator(this, this->_insert_after(this->_create_node(value), const_cast<_Node*>(pos.node)));
        }
        ...
        iterator insert_after(const_iterator pos, size_type count, const_reference value)
            ...
            for (size_type i = 0; i < count; i++) {
                this->_insert_after(this->_create_node(value), pos_node);
            }
        ...
        iterator insert_after(const_iterator pos, InputIt first, InputIt last) {
            ...
            for (InputIt input_it = first; input_it != last; ++input_it) {
                this->_insert_after(this->_create_node(*input_it), pos_node);
                pos_node = pos_node->next;
            }
        ...
        constexpr iterator insert_after(const_iterator pos, node_type&& node) noexcept {
            // If the iterator is not iterating over this singly_list instance, the iterator is at the end of the list,
            // or the node being inserted is empty...
            if (pos.parent != this || pos.node == nullptr || node.node == nullptr) {
                // Return an iterator at the end of the list
                return this->end();
            }

            iterator it;
            _Node* pos_node = std::remove_const_t<_Node*>(pos.node);

            // Insert the node at the current position
            this->_insert_after(pos_node, node._release());
            this->sz++;

            // Return an iterator at the newly inserted node
            it.node = pos_node;
            return it;
        }
        ...
        void splice_after(const_iterator pos, singly_list& other) {
            ...
            this->_splice_after(const_cast<_Node*>(pos.node), other.head, nullptr, other.sz);
        }
        ...
        void splice_after(const_iterator pos, singly_list& other, const_iterator it) {
            if (pos.node == nullptr || it.node == nullptr || it.node->next == nullptr) {
                throw std::runtime_error("segmentation fault");
            }
            ...
            this->_splice_after(std::remove_const_t<_Node*>(pos.node),
                                std::remove_const_t<_Node*>(it.node),
                                std::remove_const_t<_Node*>(it.node->next),
                                other.sz);
        }
        ...
        void splice_after(const_iterator pos, singly_list& other, const_iterator first, const_iterator last) {
            ...
            this->_splice_after(std::remove_const_t<_Node*>(pos.node),
                                std::remove_const_t<_Node*>(first.node),
                                std::remove_const_t<_Node*>(last.node),
                                other.sz);
        }
        ...
        node_type extract_after(const_iterator& pos) {
            ...
            // Return an empty node handle  if the iterator is not iteratoring over *this or
            // if there is only node in the list
            if (pos.parent != this || pos.node->next == nullptr) {
                return node_type();
            }

            // Extract the node after the position node
            node_type node_handle;
            node_handle.node = this->_extract_after(std::remove_const_t<_Node*>(pos.node));
            return node_handle;
        }
        ...
        constexpr void append_range(R&& range) noexcept {
            ...
            for (auto value : range) {
                this->_insert_after(this->_create_node(value), node);
                node = node->next;
            }
        }
